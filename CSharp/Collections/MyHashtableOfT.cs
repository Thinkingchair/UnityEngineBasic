...
namespace Collections
{
        ...
        where TValue : IEquatable<TValue>
    {
        ...
        {
            public KeyValuePair<TKey, TValue> Current => _data._buckets[_bucketindex][_itemindex];

            object IEnumerator.Current => throw new NotImplementedException();

            private MyHashtable<TKey, TValue> _data;
            private int _bucketindex; //현재 몇번째 버킷인가
            ...
            public bool MoveNext()
            {
                // 끝까지 이미 탐색을 완료햇으면
                //버킷인덱스가 초과하거나
                //마지막 버킷이면서 아이켐인덱스가 초과한경우
                if(_bucketindex > _data._validIndexList.Count  - 1)
                {
                    return false;
                }
                _bucketindex++;//다음아이템으로
                //아이템인덱스 초과시
                if (_itemindex > _data._buckets[_data._validIndexList[_bucketindex]].Count - 1)
                {
                    _itemindex = 0;//다음버킷으로
                    _bucketindex++;//넘어간 버킷의 첫번째아이템으로
                }
                return _bucketindex < _data._validIndexList.Count;//다음아이템 유효한지
            }
